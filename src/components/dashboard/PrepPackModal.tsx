...
export default function PrepPackModal(){
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      ...
      setApps(j2.map((x:any)=> ({ id: x.id, label: x.school?.name || "School" })));
    })();
  },[open]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={()=> setOpen(false)}>
          <div className="w-full max-w-2xl rounded-lg border bg-card p-4" onClick={(e)=>e.stopPropagation()}>
            ...
            <div className="mt-3 grid gap-2 text-sm">
              <select className="w-full rounded-md border px-3 py-2" value={recommenderId} onChange={e=>setRecommenderId(e.target.value)}>
                <option value="">Select recommender…</option>
                {recs.map(o=> <option key={o.id} value={o.id}>{o.label}</option>)}
              </select>
              <select className="w-full rounded-md border px-3 py-2" value={applicationId} onChange={e=>setApplicationId(e.target.value)}>
                <option value="">Select application…</option>
                {apps.map(o=> <option key={o.id} value={o.id}>{o.label}</option>)}
              </select>
              <div className="flex gap-2">
                <button className="btn btn-primary text-xs" onClick={generate} disabled={loading || !recommenderId || !applicationId}>
