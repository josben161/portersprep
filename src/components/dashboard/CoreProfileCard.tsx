    ...
    <section className="card p-4">
      ...
      <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
        <input
          ...
          placeholder="Name"
          value={p.name ?? ""}
          onChange={(e) => setP((v: any) => ({ ...v, name: e.target.value }))}
        />
        <input
          className="rounded-md border px-3 py-2"
          ...
          value={p.industry ?? ""}
          onChange={(e) =>
            setP((v: any) => ({ ...v, industry: e.target.value }))
          }
        />
        <input
          className="rounded-md border px-3 py-2"
          ...
          value={p.years_exp ?? ""}
          onChange={(e) =>
            setP((v: any) => ({ ...v, years_exp: e.target.value }))
          }
        />
        <input
          className="rounded-md border px-3 py-2"
          placeholder="GPA"
          value={p.gpa ?? ""}
          onChange={(e) => setP((v: any) => ({ ...v, gpa: e.target.value }))}
        />
        <input
          className="rounded-md border px-3 py-2"
          placeholder="GMAT"
          value={p.gmat ?? ""}
          onChange={(e) => setP((v: any) => ({ ...v, gmat: e.target.value }))}
        />
        <input
          className="sm:col-span-2 rounded-md border px-3 py-2"
          placeholder="Goals (1–2 lines)"
          value={p.goals ?? ""}
          onChange={(e) => setP((v: any) => ({ ...v, goals: e.target.value }))}
        />
      </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                  {p.resume_filename || getFileName(p.resume_key) || "CV"}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <label
                  className={`btn btn-outline text-xs ${uploading ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                    ...
                    onChange={(e) => {
                      ...
                      if (f) {
                        ...
                        if (!allowedExtensions.includes(ext || "")) {
                          setMessage({
                            type: "error",
                            text: `Please select a PDF, Word document, or text file. Got: .${ext}`,
                          });
                          return;
                        }
                        uploadCV(f);
        ...
        <div className="mt-4 p-3 rounded-md bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
          ...
          {p?.resume_analysis ? (
            <div className="space-y-2 text-xs">
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">
                  {p.resume_analysis.mbaReadiness?.fit_score || "N/A"}%
                </span>
              </div>
              ...
              {p.resume_analysis.experience && (
                <div>
                  ...
                  <span className="ml-1">
                    {p.resume_analysis.experience.years} years
                  </span>
              ...
              {p.resume_analysis.education && (
                <div>
                  ...
                  <span className="ml-1">
                    {p.resume_analysis.education.major} at{" "}
                    {p.resume_analysis.education.institution}
                  </span>
                  {p.resume_analysis.education.gpa && (
                    <span className="ml-2">
                      GPA: {p.resume_analysis.education.gpa}
                    </span>
                  )}
                </div>
              )}
            ...
            <div className="text-xs text-muted-foreground">
              Click "Analyze" to get AI-powered resume insights for your MBA
              applications.
            </div>
          )}
        </div>
              ...
              if (e.key === "Enter") {
                ...
                try {
                  ...
                  if (r.ok) {
                    ...
                    setMessage({
                      type: "error",
                      text: `Failed to add story: ${errorText}`,
                    });
                  }
                } catch (error) {
