...
export default function RequirementsPanel({
  school, applicationId, onStart
}:{ school: any; applicationId: string; onStart?: (answerId:string)=>void }) {
  ...
  async function startDraft(essay:any) {
    ...
    if (r.ok) {
      ...
      onStart?.(j.id);
      router.push(`/dashboard/applications/${applicationId}/ide?answer=${j.id}`);
    } else {
    ...
    <div className="space-y-2 text-sm">
      ...
      {essays.length === 0 ? (
        ...
        <div key={i} className="rounded-md border p-2">
          ...
          <div className="mt-2">
            <button className="btn btn-outline text-xs" onClick={()=> startDraft(e)}>Start Draft</button>
          </div>
        </div>
      ))}
