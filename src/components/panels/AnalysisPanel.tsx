  ...
  if (!analysis)
    return (
      <div className="text-sm text-muted-foreground">
        Run "Analyze" to see school focus, narrative fit, sentence-level
        signals, and evidence gaps.
      </div>
    );
  ...
  return (
    <div className="space-y-4 text-sm">
      <section>
        ...
        <div className="mt-1">
          Score: <span className="font-medium">{af?.score ?? 0}/5</span>
        </div>
        <ul className="mt-1 ml-4 list-disc">
          {(af?.hits ?? []).map((x: string) => (
            <li key={x} className="text-green-700 dark:text-green-300">
              {x}
            </li>
          ))}
        </ul>
        <ul className="mt-1 ml-4 list-disc">
          {(af?.misses ?? []).map((x: string) => (
            <li key={x} className="text-amber-700 dark:text-amber-300">
              {x}
            </li>
          ))}
        </ul>
      ...
      <section>
        ...
        <div className="mt-1">
          Cohesion: <span className="font-medium">{nf?.cohesion ?? 0}/5</span>
        </div>
        <ul className="mt-1 ml-4 list-disc">
          {(nf?.gaps ?? []).map((x: string) => (
            <li key={x}>{x}</li>
          ))}
        </ul>
      </section>
          ...
          {(analysis?.sentences ?? []).map((s: any) => (
            <div key={s.idx} className="rounded-md border p-2">
              ...
              <ul className="mt-1 ml-4 list-disc text-xs">
                {(s.suggestions ?? []).map((sg: any, i: number) => (
                  <li key={i}>
                    <span className="font-medium">{sg.type}:</span> {sg.text}
                  </li>
                ))}
              </ul>
            </div>
