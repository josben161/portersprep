...
export default function SchoolBriefPanel({
  brief,
  schoolName,
}: {
  brief?: any;
  ...
  useEffect(() => {
    ...
    fetch("/api/predict")
      ...
      .then((p) => {
        if (p?.result?.schools) {
          const schoolPred = p.result.schools.find(
            (s: any) => s.school?.toLowerCase() === schoolName?.toLowerCase(),
          );
  ...
  return (
    <div className="space-y-4 text-sm">
            ...
            <div className="flex items-center justify-between">
              <span
                className={`rounded-full px-2 py-1 text-xs font-medium ${
                  prediction.band === "reach"
                    ? "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-200"
                    : prediction.band === "target"
      ...
      <div>
        ...
        {!brief ? (
          <div className="text-muted-foreground">
            No brief loaded. Add a brief in your school data to help the AI
            match tone/values.
          </div>
        ) : (
