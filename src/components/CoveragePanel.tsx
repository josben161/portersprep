...
export default function CoveragePanel({
  ...
  useEffect(() => {
    fetch(`/api/coverage?applicationId=${applicationId}`)
      .then((r) => r.json())
      .then(setData)
      .catch(() => {});
  }, [applicationId]);
          ...
          <tbody>
            {stories.map((st: any) => (
              <tr key={st.id} className="border-t">
                ...
                {schools.map((sc: any) => {
                  const used = !!st.usage?.find(
                    (u: any) => u.school_id === sc.id,
                  );
                  return (
                    <td key={sc.id} className="p-2">
                      {used ? (
                        <div className="h-2 w-6 rounded bg-brand-500" />
                      ) : (
                        <div className="h-2 w-6 rounded bg-black/10 dark:bg-white/10" />
                      )}
                    </td>
                  );
                })}
