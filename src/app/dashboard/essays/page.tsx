...
export const metadata = { title: "Essays — PortersPrep" };
    ...
    <div className="mx-auto max-w-6xl p-6">
      ...
      {docs.length === 0 ? (
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {docs.map(d => (
            <a key={d.id} href={`/dashboard/essays/${d.id}`} className="rounded-lg border bg-card p-4 shadow-sm hover:shadow-md transition">
              <div className="font-medium">{d.title}</div>
              <div className="text-xs text-muted-foreground mt-1">Updated {new Date(d.updated_at).toLocaleString()}</div>
              <div className="text-xs text-muted-foreground mt-1">{d.word_count} words • {d.status}</div>
            </a>
          ))}
        </div>
      )}
