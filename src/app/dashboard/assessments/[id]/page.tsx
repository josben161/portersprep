...
export default async function AssessmentDetail({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Created {new Date(a.created_at).toLocaleString()}</p>

      <section className="mt-6 rounded-lg border p-4">
        <h2 className="font-medium">Admit bands</h2>
        <pre className="mt-2 overflow-auto rounded bg-muted/40 p-3 text-sm">{JSON.stringify(res.bands ?? {}, null, 2)}</pre>
      </section>

      <section className="mt-4 rounded-lg border p-4">
        <h2 className="font-medium">Angles</h2>
        <ul className="mt-2 list-disc pl-5 text-sm text-muted-foreground">
          {(res.angles ?? []).map((x: string) => <li key={x}>{x}</li>)}
        </ul>
      </section>
      ...
      <section className="mt-4 rounded-lg border p-4">
        ...
        <ul className="mt-2 list-disc pl-5 text-sm text-muted-foreground">
          {(res.gaps ?? []).map((x: string) => <li key={x}>{x}</li>)}
        </ul>
        <ul className="mt-2 list-disc pl-5 text-sm text-muted-foreground">
          {(res.timeline ?? []).map((x: string) => <li key={x}>{x}</li>)}
        </ul>
      </section>

      <a href="/dashboard/assessments/new" className="mt-6 inline-block rounded-md border px-4 py-2">Start another</a>
    </div>
  );
} 