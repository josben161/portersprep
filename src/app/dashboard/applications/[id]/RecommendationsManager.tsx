...
export default function RecommendationsManager({ appId }: { appId: string }) {
  ...
  const [schoolRequirements, setSchoolRequirements] =
    useState<SchoolRequirements | null>(null);
  ...
  async function loadData() {
    try {
      ...
      const assignmentsRes = await fetch(
        `/api/applications/${appId}/recommendations`,
      );
      ...
      <div className="border-b pb-4">
        ...
        {schoolRequirements && (
          <div className="mt-2 text-sm text-muted-foreground">
            {schoolRequirements.school_name} requires {schoolRequirements.count}{" "}
            recommendation{schoolRequirements.count > 1 ? "s" : ""} (
            {schoolRequirements.format})
          </div>
        )}
      </div>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      {assignment.recommenders.title &&
                        `${assignment.recommenders.title} at `}
              ...
              >
                ...
                {recommenders.map((recommender) => (
                  <option key={recommender.id} value={recommender.id}>
                    {recommender.name} -{" "}
                    {recommender.relationship || "No relationship specified"}
                  </option>
                ))}
              </select>
            </div>
