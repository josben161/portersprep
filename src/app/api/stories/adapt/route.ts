...
export async function POST(req: Request) {
  ...
  if(!body?.storyId || !body?.applicationId) return new Response("Bad Request",{status:400});
  ...
  // load story
  const { data: story } = await sb.from("anchor_stories").select("*").eq("id", body.storyId).eq("user_id", p.id).single();
  if(!story) return new Response("Not found", { status: 404 });

  // load school brief (if any) from DB schools or questions metadata
  const { data: schoolRow } = await sb.from("schools").select("id,slug,name,brief").eq("id", app.school_id).single();
  const questions = await listSchoolQuestions(app.school_id);

  const system = "You are an ethical MBA essay coach. You adapt stories without inventing facts.";
...
- If a metric is missing, ask for [ADD METRIC] rather than inventing it.`;
  ...
  const style = { tone: body?.tone ?? null, focus: body?.focusTags ?? [], word_limit: body?.wordLimit ?? null };
  const { data: variant, error } = await sb.from("story_variants").upsert({
    story_id: story.id, application_id: app.id, school_id: app.school_id,
    tone_profile: body?.tone ?? null, style, adapted_text: adapted
  }, { onConflict:"story_id,application_id" }).select("*").single();
