...
function generateMockPrediction(profile: any) {
  ...
  const schools = MOCK_SCHOOLS.map((school) => ({
    ...
    confidence: Math.min(
      0.95,
      baseConfidence + confidenceBoost + (Math.random() * 0.2 - 0.1),
    ),
  }));
...
export async function POST() {
  try {
    ...
    if (profileError) {
      ...
      return new Response(`Failed to load profile: ${profileError.message}`, {
        status: 500,
      });
    }
    ...
    if (insertError) {
      ...
      if (insertError.code === "42501") {
        ...
        return new Response(
          `Failed to save prediction: ${insertError.message}`,
          { status: 500 },
        );
      }
    }
    ...
    if (error instanceof Error) {
      return new Response(`Prediction failed: ${error.message}`, {
        status: 500,
      });
    } else {
