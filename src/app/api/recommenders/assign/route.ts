...
export async function POST(req: NextRequest) {
  ...
  if (!recommender_id || !application_id) return new Response("Missing fields", { status: 400 });

  const sb = getAdminSupabase();

  // Verify ownership of application
  const { data: app } = await sb.from("applications")
    .select("id,user_id").eq("id", application_id).single();
  if (!app || app.user_id !== profile.id) return new Response("Not found", { status: 404 });

  // Verify recommender belongs to user
  const { data: rec } = await sb.from("recommenders")
    .select("id,user_id").eq("id", recommender_id).single();
  if (!rec || rec.user_id !== profile.id) return new Response("Not found", { status: 404 });
