...
const Intake = z.object({
  ...
  gre: z.object({ verbal:z.number().optional(), quant:z.number().optional(), aw:z.number().optional() }).optional(),
...
- "overall": { "strengths": ["..."], "risks": ["..."], "strategy": ["..."] }
  ...
  const r = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role:"system", content:"Respond with strictly valid JSON. No markdown."},
  ...
  catch { parsed = { schools: [], overall: { strengths:[], risks:[], strategy:[] }}; }
