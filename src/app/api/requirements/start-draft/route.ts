...
import { upsertSchool, addSchoolQuestion, ensureAnswer, getApplication, listSchoolQuestions } from "@/lib/apps";
...
function inferArchetype(title: string, prompt: string): string {
  ...
  if (t.includes("why stanford") || t.includes("why booth") || t.includes("why tuck") || t.includes("why wharton") || t.includes("why school")) return "why_school";
  if (t.includes("why mba") || t.includes("post-mba") || t.includes("career goals") || t.includes("goal")) return "goals";
...
export async function POST(req: Request) {
  ...
  if (!appId || !schoolId || typeof essayIndex !== "number") return new Response("Bad Request", { status: 400 });
  ...
  const existing = qList.find((q: any) => q.prompt.trim() === essay.prompt.trim());
  const questionId = existing?.id || await addSchoolQuestion(dbSchoolId, { 
    prompt: essay.prompt, 
    archetype, 
