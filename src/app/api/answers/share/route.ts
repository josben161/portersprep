...
export async function POST(req: NextRequest) {
  ...
  const { data: ans } = await sb.from("application_answers")
    ...
    .eq("id", answer_id).single();
  if (!ans) return new Response("Not found", { status: 404 });
  
  const { data: app } = await sb.from("applications")
    .select("user_id")
    .eq("id", ans.application_id).single();
  if (!app || (app as any).user_id !== profile.id) return new Response("Not found", { status: 404 });

  const token = newToken(24);
  const { data, error } = await sb.from("answer_share_links").insert({
    answer_id,
    token,
    role,
  ...
  return Response.json({ token: data.token, url: `${process.env.NEXT_PUBLIC_APP_URL}/share/${data.token}` });
} 