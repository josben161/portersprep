...
export async function analyzeOnboardingStatus(
  ...
  try {
    ...
    const hasAdvancedApplications = context.applications.some(
      (app) => app.status && app.status !== "just_started",
    );
    ...
    const criticalMissing = steps.filter(
      (step) => step.priority === "critical" && !step.completed,
    );
...
async function generateOnboardingSteps(
  context: any,
  ...
  if (
    !context.profile.academic_interests ||
    context.profile.academic_interests.trim() === ""
  ) {
    steps.push({
      id: "profile-academic",
  ...
  if (
    !context.profile.extracurriculars ||
    context.profile.extracurriculars.trim() === ""
  ) {
    steps.push({
  ...
  steps.forEach((step) => {
    switch (step.id) {
      case "profile-name":
        step.completed = !!(
          context.profile.name && context.profile.name.trim() !== ""
        );
        break;
      case "profile-email":
        step.completed = !!(
          context.profile.email && context.profile.email.trim() !== ""
        );
        break;
      case "profile-bio":
        step.completed = !!(
          context.profile.bio && context.profile.bio.trim() !== ""
        );
        break;
      ...
      case "profile-industry":
        step.completed = !!(
          context.profile.industry && context.profile.industry.trim() !== ""
        );
        break;
      ...
      case "profile-goals":
        step.completed = !!(
          context.profile.goals && context.profile.goals.trim() !== ""
        );
        break;
