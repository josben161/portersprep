...
export async function gatherUserContext(userId: string) {
  ...
  try {
    ...
    const progress = {
      ...
      completedEssays: essays.filter(essay => essay.status === 'completed').length,
      totalRecommendations: recommendations.length,
      completedRecommendations: recommendations.filter(rec => rec.status === 'completed').length,
    ...
    const context = {
      ...
      schools: {
        ...
        requirements: applications.map(app => app.school_requirements).filter(Boolean),
    ...
    return {
      user: { profile: {}, applications: [], essays: [], recommendations: [], progress: {} },
      schools: { targetSchools: [], requirements: [], deadlines: [] },
      session: { conversationHistory: [], userPreferences: {}, learningMemory: [] }
    };
  }
}

function calculateProfileCompleteness(profile: any): number {
  const fields = ['name', 'email', 'bio', 'academic_interests', 'extracurriculars'];
