...
export async function generateSchoolPredictions(userId: string, schoolIds: string[]): Promise<SchoolPrediction[]> {
  ...
  try {
    ...
    for (const schoolId of schoolIds) {
      ...
      const prediction = await analyzeSchoolFit(context, school, resumeAnalysis);
...
Provide a comprehensive improvement plan in JSON format.`;
    ...
    return {
      ...
      estimated_impact: plan.estimated_impact || 'Impact assessment unavailable'
    };
  } catch (error) {
...
export function calculateOverallAdmissionProbability(predictions: SchoolPrediction[]): number {
  if (predictions.length === 0) return 0;
  
  const totalProbability = predictions.reduce((sum, pred) => sum + pred.admission_probability, 0);
  return Math.round(totalProbability / predictions.length);
}

export function getConfidenceLevel(predictions: SchoolPrediction[]): 'low' | 'medium' | 'high' {
  ...
  if (confidenceCounts.high > confidenceCounts.medium && confidenceCounts.high > confidenceCounts.low) {
    return 'high';
