...
class AIContextManager {
  ...
  async storeMemory(
    userId: string,
    memory: Omit<AIMemory, "id" | "user_id" | "created_at" | "updated_at">,
  ): Promise<void> {
    if (!isSupabaseAvailable()) {
  ...
  private calculateProgress(
    ...
    const essayProgress =
      essays.length > 0
        ? (essays.filter((essay) => essay.status === "completed").length /
            essays.length) *
          100
        : 0;
    const recommendationProgress =
      recommendations.length > 0
        ? (recommendations.filter((rec) => rec.status === "completed").length /
