...
export async function analyzeUserPersonality(userId: string): Promise<PersonalizationInsights> {
  ...
  try {
    ...
    const communicationStyle = determineCommunicationStyle(avgMessageLength, topics);
    
    // Determine motivational style
    const motivationalStyle = determineMotivationalStyle(memoryData, profileData);
    ...
    const recommendations = generateRecommendations(strengths, weaknesses, topics);
    ...
    return {
      ...
      recommendations: ['Focus on time management', 'Practice essay writing regularly'],
...
export async function getPersonalizedGreeting(userId: string): Promise<string> {
  ...
  const phrases = motivationalPhrases[insights.motivationalStyle as keyof typeof motivationalPhrases] || motivationalPhrases.encouraging;
...
function extractTopics(conversations: any[]): string[] {
  ...
  conversations.forEach(conv => {
    ...
    if (message.includes('school') || message.includes('college')) topics.add('school selection');
    if (message.includes('recommendation')) topics.add('recommendations');
    if (message.includes('deadline') || message.includes('timeline')) topics.add('timeline');
    if (message.includes('stress') || message.includes('anxiety')) topics.add('stress management');
...
function analyzeStressLevel(conversations: any[]): 'low' | 'medium' | 'high' {
  const stressKeywords = ['stress', 'anxiety', 'worried', 'nervous', 'overwhelmed', 'panic'];
  const stressCount = conversations.filter(conv => 
    stressKeywords.some(keyword => conv.message.toLowerCase().includes(keyword))
  ).length;
...
function determineCommunicationStyle(avgMessageLength: number, topics: string[]): string {
...
function determineMotivationalStyle(memory: any[], profile: any): string {
  ...
  if (profile?.bio?.includes('creative') || profile?.bio?.includes('artistic')) return 'supportive';
  return 'encouraging';
}
...
function identifyStrengths(profile: any, memory: any[]): string[] {
  ...
  if (memory.some(m => m.content?.function === 'analyze_user_progress')) strengths.push('Progress tracking');
...
function identifyWeaknesses(profile: any, memory: any[]): string[] {
  ...
  if (memory.some(m => m.content?.function === 'generate_essay_guidance')) weaknesses.push('Essay writing');
  
  return weaknesses.length > 0 ? weaknesses : ['Time management', 'Essay writing'];
}

function generateRecommendations(strengths: string[], weaknesses: string[], topics: string[]): string[] {
  const recommendations = [];
  
  if (weaknesses.includes('Profile completion')) {
    recommendations.push('Complete your profile to unlock personalized insights');
  }
  ...
  if (topics.includes('stress management')) {
    recommendations.push('Take breaks and practice stress management techniques');
  }
  ...
  return recommendations.length > 0 ? recommendations : ['Focus on time management', 'Practice essay writing regularly'];
} 