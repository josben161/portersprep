...
export function assertWithinLimit(
  kind: "schools" | "essays" | "stories" | "variants" | "ai_calls",
  snapshot: any,
) {
  const { limits, usage } = snapshot;

  if (kind === "schools") {
    if (usage.schools_count >= limits.schools_max)
      throwQuota("schools", limits.schools_max);
  }
  if (kind === "essays") {
    if (
      limits.essays_max !== "unlimited" &&
      usage.essays_count >= limits.essays_max
    )
      throwQuota("essays", limits.essays_max);
  }
  if (kind === "stories") {
    if (
      limits.story_bank_max !== "unlimited" &&
      usage.stories_count >= limits.story_bank_max
    )
      throwQuota("stories", limits.story_bank_max);
  }
  if (kind === "variants") {
  ...
  if (kind === "ai_calls") {
    if (
      limits.ai_calls_month !== "unlimited" &&
      usage.ai_calls_month >= limits.ai_calls_month
    )
      throwQuota("ai", limits.ai_calls_month);
  }
}
...
export async function logAiUse(
  profileId: string,
  feature: "ai_design" | "ai_draft" | "ai_analyze" | "ai_redline",
) {
  const sb = getAdminSupabase();
