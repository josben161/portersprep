...
export class AIInsightsEngine {
  ...
  private generateEssayInsights(): AIInsight[] {
    ...
    const completedEssays = this.essays.filter(
      (e) => e.status === "completed",
    ).length;
    const overallProgress =
      totalEssays > 0 ? (completedEssays / totalEssays) * 100 : 0;
    ...
    // Incomplete essays
    const incompleteEssays = this.essays.filter(
      (e) => e.status !== "completed",
    );
    ...
    // Word count analysis
    const essaysWithContent = this.essays.filter(
      (e) => e.word_count && e.word_limit,
    );
    if (essaysWithContent.length > 0) {
      const overLimit = essaysWithContent.filter(
        (e) => (e.word_count || 0) > (e.word_limit || 0),
      );
  ...
  private generateRecommendationInsights(): AIInsight[] {
    const insights: AIInsight[] = [];
    const pendingRecommendations = this.assignments.filter(
      (a) => a.status === "pending" || a.status === "requested",
    );
    const completedRecommendations = this.assignments.filter(
  ...
  private generateApplicationInsights(): AIInsight[] {
    const insights: AIInsight[] = [];
    const planningApps = this.applications.filter(
      (a) => a.status === "planning",
    );
    const inProgressApps = this.applications.filter(
    ...
    if (planningApps.length > 0) {
      insights.push({
        ...
        description:
          "Consider moving applications to 'In Progress' to start working on essays.",
        action: {
    ...
    if (this.applications.length === 0) {
      insights.push({
        ...
        description:
          "Add your first application to begin building your profile and essays.",
        action: {
  ...
  private generateProfileInsights(): AIInsight[] {
    ...
    const completedFields = profileFields.filter(
      (field) => this.profile[field as keyof ProfileData],
    );
